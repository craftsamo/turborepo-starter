 ...
 */
export const Toolbar = async ({ variant = 'docked' }: NavigationVariantProps) => {
      ...
      <Container>
        ...
        >
          <nav aria-label={t('primary')}>
            ...
            >
              <Image className='size-5' src={Logo} alt={t('logoAlt')} />
              ...
              {navItems
                ...
                .map((item) => (
                  <Button key={item.href} asChild variant='ghost' size='sm'>
                    <ToolbarLink href={item.href} label={t(item.label)} external={item.external} />
                  </Button>
                ))}
              <LanguageSwitcher mode='toolbar' />
              <ThemeToggle />
            </HStack>
          </nav>
        </HStack>
